...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (
      Object.keys(files).some((key) => key.startsWith("alternative_images"))
    ) {
      const alternativeImagesUploadPromises = Object.keys(files)
        ...
        .map(async (fileKey, index) => {
          ...
          await uploadFileToS3(file.filepath, altImagePath, file.mimetype);
    ...
    if (alternativeImagesUrls.length > 0) {
      ...
      updateData.alternative_images = existingProduct.alternative_images || [];
    }
